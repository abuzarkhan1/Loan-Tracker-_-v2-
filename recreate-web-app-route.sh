...
if [ "$EUID" -ne 0 ]; then
  error "Please run as root: sudo bash recreate-web-app-route.sh"
fi
...
DOMAIN="${DOMAIN:-app.marenax.site}"
SERVICE="${SERVICE:-http://localhost:5173}"
TUNNEL_NAME="${TUNNEL_NAME:-oldarena}"
LOCAL_TEST_URL="${LOCAL_TEST_URL:-http://localhost:5173/health}"
PUBLIC_TEST_URL="${PUBLIC_TEST_URL:-https://$DOMAIN/}"
